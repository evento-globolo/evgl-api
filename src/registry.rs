...
impl ProviderRegistry {
    pub fn build(config: &Config) -> anyhow::Result<Self> {
        ...
        adapters.insert(ProviderKind::Craigslist, Arc::new(CraigslistAdapter::default()));
        adapters.insert(ProviderKind::GenericWebhook, Arc::new(GenericWebhookAdapter::default()));
        ...
        Ok(Self { adapters: Arc::new(adapters) })
    }
    ...
    pub fn capabilities(&self) -> Vec<(ProviderCapabilities, bool)> {
        ProviderKind::ALL.into_iter().map(|provider| {
...
fn static_capability(provider: ProviderKind) -> ProviderCapabilities {
    ...
    match provider {
        ProviderKind::Eventbrite => ProviderCapabilities {
            provider, delivery_mode: DeliveryMode::NativeEvent, oauth: true,
            create: true, update: false, delete: false, publish: true, webhooks: true,
        ...
        ProviderKind::Meetup => ProviderCapabilities {
            provider, delivery_mode: DeliveryMode::NativeEvent, oauth: true,
            create: true, update: false, delete: false, publish: true, webhooks: false,
            requires_manual_step: false,
            notes: vec!["OAuth credentials are not configured on this deployment.".into()],
        },
        ProviderKind::MetaFacebookPage => ProviderCapabilities {
            provider, delivery_mode: DeliveryMode::DistributionPost, oauth: true,
            create: true, update: false, delete: false, publish: true, webhooks: true,
            requires_manual_step: false,
            notes: vec!["Meta OAuth credentials are not configured on this deployment.".into()],
        },
        ProviderKind::Craigslist => ProviderCapabilities {
            provider, delivery_mode: DeliveryMode::ManualHandoff, oauth: false,
            create: false, update: false, delete: false, publish: false, webhooks: false,
        ...
        ProviderKind::GenericWebhook => ProviderCapabilities {
            provider, delivery_mode: DeliveryMode::SignedWebhook, oauth: false,
            create: true, update: false, delete: false, publish: true, webhooks: false,
            requires_manual_step: false,
